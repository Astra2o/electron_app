 ...
 *
 * Windows only. Drops a `Circle to Lens.lnk` into the per-user Start
 * Menu Programs folder so the app is searchable from the Windows
 * search bar (and findable in the Start menu list).
 *
 * We do this on every app start so the shortcut auto-updates if the
 * user moves / rebuilds the app, but the operation is debounced via
 * a stamp file so we don't spend ~500ms on PowerShell every launch.
 *
 * Two modes:
 *
 ...
 *
 *   - Dev (`npm start`): TargetPath is electron.exe and Arguments is
 *     the absolute path to the app directory so the same launch
...
const SHORTCUT_NAME = "Circle to Lens.lnk";
 ...
 * We use single-quoted PS strings everywhere to dodge escaping for
...
function writeShortcut(lnkPath) {
  ...
  // PowerShell uses '' (doubled single quote) to escape a single quote
  // inside a single-quoted string. We pre-escape every path that way.
  ...
  // Ensure the Start Menu folder exists (it almost always does).
  try {
